...
def permute_image(image, axes):
    try:
        new_data = np.copy(image.get_data())
        new_data = np.transpose(new_data, axes)
    except TypeError:
        new_data = np.transpose(image.get_data(), axes)
    return new_img_like(image, data=new_data)
...
def distort_image(image, flip_axis=None, permute_axis=None, scale_factor=None):
    if flip_axis:
        image = flip_image(image, flip_axis)
    if permute_axis:
        image = permute_image(image, permute_axis)
    if scale_factor is not None:
        image = scale_image(image, scale_factor)
    return image


def augment_data(data, truth, affine=None, scale_deviation=0.25, flip=True, permute=False, gamma_delta=None):
    ...
    if flip:
        ...
        flip_axis = None
    if permute:
        permute_axis = [*np.random.permutation(n_dim)]
    else:
        permute_axis=None
    if gamma_delta:
        gamma = 1-gamma_delta + 2*gamma_delta*np.random.rand()
    ...
    for c in range(data.shape[0]):
        image = get_image(data[c], affine)
            
        data_list.append(resample_to_img(distort_image(image, flip_axis=flip_axis, permute_axis=permute_axis,
                                                           scale_factor=scale_factor), image,
                                             interpolation="continuous").get_data())    
    data = np.asarray(data_list)

    for c in range(truth.shape[0]):
        truth_image = get_image(truth[c], affine)
        
        truth_list.append(resample_to_img(distort_image(truth_image, flip_axis=flip_axis, permute_axis=permute_axis, scale_factor=scale_factor),
                                 truth_image, interpolation="nearest").get_data())

    truth_data = np.asarray(truth_list)
    np.clip(data, 0, 1, out=data)
#    print('AXIS')
#    if flip_axis is not None:        
#        print(*flip_axis, sep=', ')
#    print("DATA_SHAPE: {}".format(np.amax(data)))
#    unique, counts = np.unique(truth_data, return_counts=True)
#    print("TRUTH_SHAPE: {}".format(dict(zip(unique, counts))))
